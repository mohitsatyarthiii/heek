  ...
  const handleSingleSubmit = async (e) => {
    ...
    try {
      ...
      if (profileError || !profile) {
        throw new Error(
          "User profile not found. Contact admin to setup your account.",
        );
      }
      ...
      if (!["admin", "manager"].includes(userRole)) {
        throw new Error(
          `Only admin and manager can create creators. Your role: ${userRole}`,
        );
      }
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      if (!["admin", "manager"].includes(profile.role)) {
        throw new Error(
          `Only admin and manager can bulk create creators. Your role: ${profile.role}`,
        );
      }
        ...
        headers.forEach((header, index) => {
          ...
          switch (header.toLowerCase()) {
            ...
            case "secondary_categories":
              creator.secondary_categories = value
                ? value
                    .split(",")
                    .map((c) => c.trim())
                    .filter((c) => c)
            ...
            case "typical_deliverables":
              creator.typical_deliverables = value
                ? value
                    .split(",")
                    .map((d) => d.trim())
                    .filter((d) => d)
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-background border border-border rounded-lg overflow-hidden">
                            ...
                            <tr>
                              {Object.keys(csvPreview[0]).map(
                                (header, index) => (
                                  <th
                                    key={index}
                                    className="text-left p-3 text-sm font-semibold text-foreground border-r border-border"
                                  >
                                    {header}
                                  </th>
                                ),
                            ...
                            {csvPreview.map((row, rowIndex) => (
                              ...
                              >
                                {Object.values(row).map((value, cellIndex) => (
                                  <td
                                    key={cellIndex}
                                    className="p-3 text-sm text-foreground border-r border-border"
                                  >
                                    {value}
                                  </td>
                                ))}
                      ...
                      <div className="p-3 bg-accent border-t border-border text-sm text-muted-foreground">
                        Showing {csvPreview.length} row
                        {csvPreview.length > 1 ? "s" : ""} (first 5 rows)
                      </div>
                    </div>
                  </div>
                )}
